...
return array(
    ...
    'message_body'    => array(
        ...
        'value'        => '{$end_date=strtotime($order.create_datetime)+$delay*86400}' . "\n" .
            ...
            '<p style="font-face:sans-serif;font-size:12pt">К сожалению, оплата до сих пор не поступила. Пожалуйста, оплатите ваш заказ до {$end_date|wa_date:"humandate"}, иначе он будет отменен.</p>' . "\n\n" .
            '<p style="font-face:sans-serif;font-size:12pt">Если у вас возникли какие-то трудности с оплатой или любые другие вопросы относительно вашего заказа, напишите нам письмо на адрес <b><a href="mailto:{$wa->shop->settings("email")}">{$wa->shop->settings("email")|escape}</a></b> или позвоните по телефону <b style="white-space:nowrap">{$wa->shop->settings("phone")}</b>. Будем рады помочь.</p>' . "\n\n" .
            '<p  style="font-face:sans-serif;font-size:12pt">--<br>' . "\n" .
            '{$wa->shop->settings("name")}<br>' . "\n" .
            '{$wa->shop->settings("phone")}<br>' . "\n" .
            '{$wa->shop->settings("email")}</p>' . "\n",
